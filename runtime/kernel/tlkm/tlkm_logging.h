...
#define tlkm_log(level, fmt, ...)                                              \
	do {                                                                   \
		switch ((int)level) {                                          \
		case 0:                                                        \
			printk(KERN_ERR "tapasco [%s]: " fmt "\n", __func__,   \
			       ##__VA_ARGS__);                                 \
			break;                                                 \
		case 1:                                                        \
			printk(KERN_WARNING "tapasco [%s]: " fmt "\n",         \
			       __func__, ##__VA_ARGS__);                       \
			break;                                                 \
		default:                                                       \
			if (tlkm_logging_flags & level)                        \
				printk(KERN_NOTICE "tapasco [%s]: " fmt "\n",  \
				       __func__, ##__VA_ARGS__);               \
			break;                                                 \
		}                                                              \
	} while (0)
...
#define DEVLOG(dev_id, l, msg, ...)                                            \
	tlkm_device_log(dev_id, (l), msg, ##__VA_ARGS__)

#define tlkm_device_log(dev_id, level, fmt, ...)                               \
	do {                                                                   \
		switch ((int)level) {                                          \
		case 0:                                                        \
			printk(KERN_ERR "tapasco device #%02u [%s]: " fmt      \
					"\n",                                  \
			       dev_id, __func__, ##__VA_ARGS__);               \
			break;                                                 \
		case 1:                                                        \
			printk(KERN_WARNING "tapasco device #%02u [%s]: " fmt  \
					    "\n",                              \
			       dev_id, __func__, ##__VA_ARGS__);               \
			break;                                                 \
		default:                                                       \
...
#define DEVERR(dev_id, fmt, ...)                                               \
	printk(KERN_ERR "tapasco device #%02u [%s]: " fmt "\n", dev_id,        \
	       __func__, ##__VA_ARGS__)

#define DEVWRN(dev_id, fmt, ...)                                               \
	printk(KERN_WARNING "tapasco device #%02u [%s]: " fmt "\n", dev_id,    \
	       __func__, ##__VA_ARGS__)
